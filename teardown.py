...
from test.dq.cli.include.common import SUCCESS, e, pprint, YAML_CONFIG, CFG_FILE
from test.dq.cli.__init__ import __version__
import os, sys
from test.dq.cli.include.utils import init
import builtins
import warnings
...
from test.dq.cli.include.dq_cli import get_cli, DqCliQuitError
from test.dq.cli.include.dq_metadb import DqMetadb
from test.dq.cli.include.common import SUCCESS, e
from test.dq.cli.include.tools import format_output, OutputSettings

if YAML_CONFIG:
    from test.dq.cli.include.config import YamlConfig as Config
else:
    from test.dq.cli.include.config import PyConfig as Config

from test.dq.cli.include.common import environments, exec_type, SYNC_EXEC, ASYNC_EXEC

from test.dq.cli.include.athena_execute import AthenaExecute
from test.dq.cli.include.postgres_execute import PostgresExecute
from test.dq.cli.include.redshift_execute import RedshiftExecute
from test.dq.cli.include.snowflake_execute import SnowflakeExecute

from test.dq.cli.include.common import executors, EXEC_ATHENA, EXEC_REDSHIFT, EXEC_POSTGRES, EXEC_SNOWFLAKE


def get_executor(name, compute, conf):
    if name == executors[EXEC_ATHENA]:
        return AthenaExecute(compute=compute, conf=conf)
    elif name == executors[EXEC_REDSHIFT]:
        return RedshiftExecute(compute=compute, conf=conf)
    elif name == executors[EXEC_POSTGRES]:
        return PostgresExecute(compute=compute, conf=conf)
    elif name == executors[EXEC_SNOWFLAKE]:
        return SnowflakeExecute(compute=compute, conf=conf)
    else:
...
def cli(**kwargs):
    ...
    if 1:
        ...
        headers = ['Year' if not default_year else f'Year({default_year})',
                   'Month' if not default_month else f'Month({default_month})', 'Query ID', 'Error', 'Status']
    ...
    if 1:
        ...
        headers = ['Year' if not default_year else f'Year({default_year})',
                   'Month' if not default_month else f'Month({default_month})', 'JSON length', 'Error', 'Status']
...
def local_teardown(conn, out):
    ...
    for row in cur.execute(sql):
        # print(float(row[4]) )
        if float(row[4]) != 0:
            out.append([row[0], row[1].split('-')[1], row[1], comma_me(str(row[2])), comma_me(str(row[3])),
                        comma_me(str(row[4]))])
...
class SnowDatabaseWorker(Thread):
    ...
    def __init__(self, cmd, year, result_queue, db_meta):
        ...
        my_env["SNOWSQL_USER"] = 'test.etl.dev@test.com'
...
def submit_athena_async(num_years, year_queue, db_meta, usecase):
    ...
    if default_year:
        ...
        cmd = f"""aws athena start-query-execution --query-string "{q}" --region '{db_meta[
            'region_name']}' --result-configuration "OutputLocation={db_meta['s3_staging_dir']}" """
...
if __name__ == "__main__":
    ...
    if 0:
        ...
        headers = ['Year' if not default_year else f'Year({default_year})',
                   'Month' if not default_#!python3.7.2
# -*- coding: utf-8 -*-
